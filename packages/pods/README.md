...
`void` simplifies running large language models on remote GPU pods. It automatically:
...
export VOID_API_KEY=your_api_key              # Any string you want for API authentication

# Setup a DataCrunch pod with NFS storage (models path auto-extracted)
void pods setup dc1 "ssh root@1.2.3.4" \
...
void start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen

# Send a single message to the model
void agent qwen "What is the Fibonacci sequence?"
...
export OPENAI_API_KEY=$VOID_API_KEY
```
...
```bash
void pods setup <name> "<ssh>" [options]        # Setup new pod
...
void pods                                       # List all configured pods
void pods active <name>                         # Switch active pod
void pods remove <name>                         # Remove pod from local config
void shell [<name>]                             # SSH into pod
void ssh [<name>] "<command>"                   # Run command on pod
```
...
```bash
void start <model> --name <name> [options]  # Start a model
...
void stop [<name>]          # Stop model (or all if no name given)
void list                   # List running models with status
void logs <name>            # Stream model logs (tail -f)
```

### Agent & Chat Interface

```bash
void agent <name> "<message>"               # Single message to model
void agent <name> "<msg1>" "<msg2>"         # Multiple messages in sequence
void agent <name> -i                        # Interactive chat mode
void agent <name> -i -c                     # Continue previous session

# Standalone OpenAI-compatible agent (works with any API)
void-agent --base-url http://localhost:8000/v1 --model llama-3.1 "Hello"
void-agent --api-key sk-... "What is 2+2?"  # Uses OpenAI by default
void-agent --json "What is 2+2?"            # Output event stream as JSONL
void-agent -i                                # Interactive mode
```
...
`void` includes predefined configurations for popular agentic models, so you do not have to specify `--vllm` arguments manually. `void` will also check if the model you selected can actually run on your pod with respect to the number of GPUs and available VRAM. Run `void start` without additional arguments to see a list of predefined models that can run on the active pod.
...
void start Qwen/Qwen3-Coder-30B-A3B-Instruct --name qwen3

# Qwen3-Coder-480B - State-of-the-art on 8xH200 (data-parallel mode)
void start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen-480b
```

### GPT-OSS Models
```bash
# Requires special vLLM build during setup
void pods setup gpt-pod "ssh root@1.2.3.4" --models-path /workspace --vllm gpt-oss

# GPT-OSS-20B - Fits on 16GB+ VRAM
void start openai/gpt-oss-20b --name gpt20

# GPT-OSS-120B - Needs 60GB+ VRAM
void start openai/gpt-oss-120b --name gpt120
```

### GLM Models
```bash
# GLM-4.5 - Requires 8-16 GPUs, includes thinking mode
void start zai-org/GLM-4.5 --name glm

# GLM-4.5-Air - Smaller version, 1-2 GPUs
void start zai-org/GLM-4.5-Air --name glm-air
```
...
void start deepseek-ai/DeepSeek-V3 --name deepseek --vllm \
  --tensor-parallel-size 4 --trust-remote-code

# Mistral with pipeline parallelism
void start mistralai/Mixtral-8x22B-Instruct-v0.1 --name mixtral --vllm \
  --tensor-parallel-size 8 --pipeline-parallel-size 2

# Any model with specific tool parser
void start some/model --name mymodel --vllm \
...
void pods setup dc1 "ssh root@instance.datacrunch.io" \
  --mount "sudo mount -t nfs -o nconnect=16 nfs.fin-02.datacrunch.io:/your-pseudo /mnt/hf-models"
...
```bash
# With network volume
void pods setup runpod "ssh root@pod.runpod.io" --models-path /runpod-volume

# Or use workspace (persists with pod but not shareable)
void pods setup runpod "ssh root@pod.runpod.io" --models-path /workspace
```
...
When running multiple models, void automatically assigns them to different GPUs:
```bash
void start model1 --name m1  # Auto-assigns to GPU 0
void start model2 --name m2  # Auto-assigns to GPU 1
void start model3 --name m3  # Auto-assigns to GPU 2
```
...
void start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen --gpus 1

# Run GLM-4.5 on 8 GPUs (if it has an 8-GPU config)
void start zai-org/GLM-4.5 --name glm --gpus 8
```
...
void start meta-llama/Llama-3.1-70B-Instruct --name llama70b --vllm \
  --tensor-parallel-size 4

# Specific GPU count
void start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen480 --vllm \
...
`void` includes a standalone OpenAI-compatible agent that can work with any API:

```bash
# Install globally to get void-agent command
bun add -g @void/pods

# Use with OpenAI
void-agent --api-key sk-... "What is machine learning?"

# Use with local vLLM
void-agent --base-url http://localhost:8000/v1 \
...
void-agent --continue "Follow up question"

# Custom system prompt
void-agent --system-prompt "You are a Python expert" "Write a web scraper"

# Use responses API (for GPT-OSS models)
void-agent --api responses --model openai/gpt-oss-20b "Hello"
```
...
`void` automatically configures appropriate tool calling parsers for known models:
...
```bash
void start model --name mymodel --vllm --disable-tool-call-parser
```
...
void start Qwen/Qwen2.5-Coder-32B-Instruct --name coder \
...
Sessions are stored in `~/.void/sessions/` organized by project path and include:
...
```bash
void-agent --api-key sk-... --json "What is 2+2?"
```
...
- Check CUDA compatibility with `void ssh "nvidia-smi"`
...
```bash
# Good
void agent qwen "What is this file about?"
...
void start model --name test --pod dev-pod
void list --pod prod-pod
void stop test --pod dev-pod
```

### Custom vLLM Arguments
```bash
# Pass any vLLM argument after --vllm
void start model --name custom --vllm \
  --quantization awq \
...
void ssh "watch -n 1 nvidia-smi"

# Check model downloads
void ssh "du -sh ~/.cache/huggingface/hub/*"

# View all logs
void ssh "ls -la ~/.vllm_logs/"
...
- `VOID_API_KEY` - API key for vLLM endpoints
- `VOID_CONFIG_DIR` - Config directory (default: `~/.void`)
- `OPENAI_API_KEY` - Used by `void-agent` when no `--api-key` provided
