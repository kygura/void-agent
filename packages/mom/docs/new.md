...
```typescript
import { type AgentSessionEvent } from "@void/coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@void/mom";
...
2. `~/.void/mom/tools/**/index.ts` (global, recursive)
...
async function loadCustomTools(dataDir: string): Promise<LoadedTool[]> {
  ...
  const toolDirs = [
    ...
    path.join(os.homedir(), ".void", "mom", "tools"),
  ];
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@void/ai` and `@void/mom` are aliased to the running mom bundle.
