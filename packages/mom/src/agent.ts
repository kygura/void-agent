import { Agent, type AgentEvent } from "@void/agent";
import { getModel, type ImageContent } from "@void/ai";
import {
	AgentSession,
	AuthStorage,
	convertToLlm,
	...
	if (!key) {
		throw new Error(
			"No API key found for anthropic.\n\n" +
				...
				join(homedir(), ".void", "mom", "auth.json"),
		);
	}
	return key;
}
...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	const authStorage = AuthStorage.create(join(homedir(), ".void", "mom", "auth.json"));
