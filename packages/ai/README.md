...
TypeBox exports are re-exported from `@void/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@void/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@void/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@void/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@void/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@void/ai';
...
```typescript
import { getModel, stream } from '@void/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@void/ai';
...
```typescript
import { Model, stream } from '@void/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@void/ai';
...
```typescript
import { getModel, complete, Context } from '@void/ai';
...
```typescript
import { Context, getModel, complete } from '@void/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@void/ai/oauth` entry point in Node.js.
...
Set `VOID_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export VOID_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `VOID_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `VOID_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
```bash
bunx @void/ai login              # interactive provider selection
bunx @void/ai login anthropic    # login to specific provider
bunx @void/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@void/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@void/ai/oauth';
...
```typescript
import { getModel, complete } from '@void/ai';
import { getOAuthApiKey } from '@void/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@void/ai`
