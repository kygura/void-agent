...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @void/ai <command> [provider]
...
Examples:
  npx @void/ai login              # interactive provider selection
  npx @void/ai login anthropic    # login to specific provider
  npx @void/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @void/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @void/ai --help' for usage`);
	process.exit(1);
}
