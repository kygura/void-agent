...
describe("Cache Retention (VOID_CACHE_RETENTION)", () => {
	const originalEnv = process.env.VOID_CACHE_RETENTION;

	beforeEach(() => {
		delete process.env.VOID_CACHE_RETENTION;
	});

	afterEach(() => {
		if (originalEnv !== undefined) {
			process.env.VOID_CACHE_RETENTION = originalEnv;
		} else {
			delete process.env.VOID_CACHE_RETENTION;
		}
	});
	...
	describe("Anthropic Provider", () => {
		it.skipIf(!process.env.ANTHROPIC_API_KEY)(
			"should use default cache TTL (no ttl field) when VOID_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.ANTHROPIC_API_KEY)("should use 1h cache TTL when VOID_CACHE_RETENTION=long", async () => {
			process.env.VOID_CACHE_RETENTION = "long";
			const model = getModel("anthropic", "claude-haiku-4-5-20251001");
			let capturedPayload: any = null;
		...
		it("should not add ttl when baseUrl is not api.anthropic.com", async () => {
			...
			const baseModel = getModel("anthropic", "claude-haiku-4-5-20251001");
			const proxyModel = {
				...baseModel,
	...
	describe("OpenAI Responses Provider", () => {
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should not set prompt_cache_retention when VOID_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should set prompt_cache_retention to 24h when VOID_CACHE_RETENTION=long",
			async () => {
				process.env.VOID_CACHE_RETENTION = "long";
				const model = getModel("openai", "gpt-4o-mini");
				let capturedPayload: any = null;
