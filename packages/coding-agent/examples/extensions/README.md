...
Example extensions for void-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
void --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.void/extensions/
```
...
| `send-user-message.ts` | Demonstrates `void.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs void theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `void.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using void-ai's built-in Anthropic/OpenAI streaming via proxy |
