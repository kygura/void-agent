 ...
 * - ~/.void/presets.json (global)
 * - <cwd>/.void/presets.json (project-local)
 *
...
import type { ExtensionAPI, ExtensionContext } from "@void/coding-agent";
import { DynamicBorder, getAgentDir } from "@void/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@void/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".void", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.void/presets.json or .void/presets.json", "warning");
			return;
		}
