...
```
~/.void/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.void/sessions/`.

void also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, void uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@void/coding-agent/dist/` and `node_modules/@void/ai/dist/`.
...
### Base Message Types (from void-ai)
...
### Extended Message Types (from void-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if void-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` command or `void.setSessionName()` in extensions.
