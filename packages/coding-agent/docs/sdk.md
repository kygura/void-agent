> void can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to void's agent capabilities. Use it to embed void in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession } from "@void/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `void.sendMessage()`.
...
The `Agent` class (from `@void/agent`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.void", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.void/extensions/`)
- Project skills:
  - `.void/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.void/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.void/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@void/coding-agent";
...
// Default: uses ~/.void/auth.json and ~/.void/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@void/coding-agent";
...
- If you omit `tools`, void automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@void/coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `void.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `void.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.void/extensions/`, `.void/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `void.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@void/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@void/coding-agent";
...
1. Global: `~/.void/settings.json`
2. Project: `<cwd>/.void/settings.json`
