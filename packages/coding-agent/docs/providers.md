...
void supports subscription-based providers via OAuth and API key providers via environment variables or auth file. For each provider, void knows all available models. The list is updated with every void release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.void/auth.json` and auto-refresh when expired.
...
Store credentials in `~/.void/auth.json`:
...
```bash
void --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
void --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
