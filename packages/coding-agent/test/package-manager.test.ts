...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.VOID_OFFLINE;
		delete process.env.VOID_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.VOID_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .void skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .void", async () => {
			const extDir = join(tempDir, ".void", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".void", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".void");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .void auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".void\n");

			const skillDir = join(tempDir, ".void", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .void settings base", () => {
			...
			const rel = relative(join(tempDir, ".void"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".void", "npm")],
				undefined,
			);
		});
		...
		it("should not fetch npm registry during resolve for installed unpinned packages", async () => {
			const installedPath = join(tempDir, ".void", "npm", "node_modules", "example");
			mkdirSync(join(installedPath, "extensions"), { recursive: true });
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".void", "npm", "node_modules", "example");
