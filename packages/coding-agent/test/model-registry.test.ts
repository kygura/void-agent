...
import type { Api, Context, Model, OpenAICompletionsCompat } from "@void/ai";
import { getApiProvider } from "@void/ai";
import { getOAuthProvider } from "@void/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `void-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
