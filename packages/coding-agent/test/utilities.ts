...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@void/ai";
...
// ============================================================================
// OAuth API key resolution from ~/.void/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".void", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.void/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.void/auth.json
 */
...
export const VOID_AGENT_DIR = join(homedir(), ".void");

/**
 * Get an AuthStorage instance backed by ~/.void/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `void-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
